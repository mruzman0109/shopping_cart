...
@Service
@Profile("mongo")
public class CartServiceMongoImpl implements CartService {
   ...
   public boolean addItem(String customerId, CartItemDTO item) {
      ...
      cartRepository.save(CartMapper.toMongo(cart));
      return true;
   }
   ...
   public boolean removeItem(String customerId, String itemId) {
      ...
      cart.getItems().stream().map(item -> {
         ...
         return item;
      }).forEach(item -> cartRepository.save(CartMapper.toMongo(cart)));
      return true;
   }
